...
fun LayoutManagerScreen(
    ...
    val filtered = KeyboardLayoutCatalog.all.filter { item ->
        ...
        (query.isBlank() || label.contains(query, true) || item.id.contains(
            query,
            true
    ...
    Scaffold(topBar = {
        TopAppBar(
            title = { Text(stringResource(R.string.manage_layouts)) },
            navigationIcon = {
        ...
        Column(
            ...
            ScrollableTabRow(
                ...
                Tab(
                    ...
                    text = { Text(stringResource(R.string.all_layouts)) })
                LayoutCategory.entries.forEach { c ->
                    Tab(
                        selected = category == c,
                        onClick = { category = c },
            ...
            LazyColumn(
                ...
                items(filtered, key = { it.id }) { item ->
                    ...
                    ListItem(
                        ...
                        supportingContent = {
                            Text(
                                item.category.name.lowercase().replaceFirstChar { it.titlecase() })
                        },
                        trailingContent = {
                            Switch(
                                checked = checked,
